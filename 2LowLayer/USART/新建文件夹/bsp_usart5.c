...
void bsp_Usart5_Init (uint32_t BaudRate)
{
     ...
     RCC_APB1PeriphClockCmd(USART5_CLK,ENABLE); //使能UART时钟
     RCC_AHB1PeriphClockCmd(USART5_GPIO_RX_CLK|USART5_GPIO_TX_CLK,ENABLE);//使能GPIO时钟
...
*****************************************************************************/
uint16_t bsp_Usart5_RecvAtTime (uint8_t *Buff, uint16_t RecvSize, uint32_t timeout_MilliSeconds)
{
	uint16_t RecvLen = 0;
	uint8_t tmp[1] = {0};
	...
	while (1)
	{
		if (g1msTimerUART5 == 0) return RecvLen;
