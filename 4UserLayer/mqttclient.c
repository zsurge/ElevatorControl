...
//   {
	...
	if ( msg->msglenth > 2 ) //只有当消息长度大于2 "{}" 的时候才去处理它
	{
		...
		{
			case 0:
				log_d ( "MQTT>>消息质量：QoS0\n" );
				break;
			case 1:
				log_d ( "MQTT>>消息质量：QoS1\n" );
				break;
			case 2:
				log_d ( "MQTT>>消息质量：QoS2\n" );
				break;
			default:
				log_d ( "MQTT>>错误的消息质量\n" );
				break;
...
MQTT_START:
	...
	//创建网络连接
	log_d ( "1.开始连接对应云平台的服务器...\n" );
	log_d ( "服务器IP地址：%s，端口号：%0d！\n",host_ip,HOST_PORT );
	while ( 1 )
	{
		//连接服务器
		...
		log_d ( "连接云平台服务器失败，等待3秒再尝试重新连接！\n" );
        
		//等待3秒
		vTaskDelay ( 3000 );
	}

	log_d ( "2.MQTT用户名与秘钥验证登陆...\n" );
	//MQTT用户名与秘钥验证登陆
	if ( MQTT_Connect() != Connect_OK )
	{
		//重连服务器
		log_d ( "MQTT用户名与秘钥验证登陆失败...\n" );
		//关闭链接
		transport_close();
		goto MQTT_START;
	}
